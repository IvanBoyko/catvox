...
- Before creating a PR, AI agents MUST evaluate if it corresponds to an existing GitHub Issue. This evaluation is mandatory, but linking is optional (e.g., chores or hotfixes may not need an issue). If there is no clear parent Issue, the agent must explicitly ask the user: "Should this PR be linked to a GitHub Issue?" before proceeding. When a PR is one slice of a larger issue or epic, reference the parent issue in the PR body with non-closing language such as `Part of #NN` or `Refs #NN`. Use closing keywords such as `Closes #NN` only on the final PR that actually completes the issue. Because GitHub does not currently expose an API to programmatically link non-closing PRs in the "Development" metadata section, **AI agents MUST explicitly remind the user in their chat response to manually associate the PR with the Issue in the GitHub UI so the user does not forget.**
