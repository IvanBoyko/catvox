...
Implementer identity in this repo is fluid — multiple developer agents (Codex, Antigravity/Gemini) have rotated through the role, with Codex also running as an automated PR reviewer under `chatgpt-codex-connector[bot]`. Don't hardcode an implementer's name into review findings or handoff phrasing; identify the current developer agent from the `_Posted by <X>_` attribution on the most recent non-Ivan commit or PR comment, and phrase handoffs role-agnostically ("hand off to the developer agent") rather than naming a specific tool.

Before checking out a PR to review, compare the current branch to the PR head. If they already match (e.g. the session-start `gitStatus` block already shows the PR branch), do nothing — no fetch, no checkout, the local tree is what the PR is for. If the current branch doesn't match, prefer `git fetch origin && git checkout <branch>` over `git fetch origin pull/<N>/head:<localname>`. All developer agents in this repo push branches directly to `origin` (`kathelix/catvox`), so the named branch is already on the remote; creating a synthetic local ref like `pr-NN-review` when the real branch is available just leaves cleanup work. Reserve the `pull/<N>/head:` form for PRs from forks where the branch is not on `origin`.
