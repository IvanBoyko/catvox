...
├── terraform/                       # Terraform roots — one subdirectory per root
│   ├── core/                        # GCP/Firebase root — state prefix catvox/state; CI terraform.yml
│   │   ├── main.tf                  # GCS bucket, Firestore, APIs, Secret Manager
│   │   ├── iam.tf                   # Service accounts and IAM bindings
│   │   ├── monitoring.tf            # Cloud Monitoring error alerting
...
│   │   ├── outputs.tf
│   │   └── README.md
│   └── README.md                    # Umbrella — Terraform layout conventions
...
`terraform/core/env/<environment>.tfvars` files hold only `app_check_debug_token` and
...
`make functions-integration` needs a Firebase App Check debug token. It preserves an explicitly supplied `CATVOX_APP_CHECK_DEBUG_TOKEN`; otherwise `TF_VAR_app_check_debug_token` is accepted by the integration script. For local developer convenience, the Makefile silently falls back to `app_check_debug_token` in the selected ignored `terraform/core/env/<environment>.tfvars` when neither environment variable is set. Never commit debug tokens or shared Xcode schemes containing `AppCheckDebugToken` or `FIRAAppCheckDebugToken`.
...
`terraform/core/env/*.tfvars` is gitignored. Copy from the matching `.example` files and fill in the remaining private values. The tfvars file is secrets-only for `app_check_debug_token` and `alert_email`. The Makefile injects the remote-state coordinates directly via inline `-backend-config` arguments.
...
| `terraform.yml` (plan) | PR touching `terraform/core/**`, `Makefile`, or workflow | fmt-check → init → validate → plan → PR comment (only when the plan has changes or a check failed) |
...
- When a change *removes* validation coverage — a CI job, a workflow `paths` filter, a guard, or a test — design its replacement in the same change instead of leaving a gap, and re-derive whether anything still validates what you stopped checking. Extracting `tools/ai-loop` to a submodule dropped its suite from `make scripts-test` and both workflow `paths` filters, leaving no CatVox-owned check on the pinned submodule revision until a dedicated `.github/workflows/ai-loop-submodule.yml` consumer gate was added (Codex finding F1, PR #140). Two corollaries from the same PR: (1) a guarantee asserted in a code comment, an ADR consequence, or a PR-body summary must match what the code actually does — after each rewire, sweep the docs and ADRs you wrote earlier (ADR-0028 and the PR body kept claiming "CatVox CI does not initialize the submodule" after the new gate made that false — finding T1); (2) a guard must actually validate everything its *trigger* implies — the gate fired on `.gitmodules` changes, but `actions/checkout` with `submodules: true` checks out the pinned commit and ignores the configured `branch`, so a branch typo passed the gate and only broke later at `git submodule update --remote` until a `git ls-remote --exit-code --heads` check was added (finding U1). Negative-test the guard (a bogus branch, a broken pointer) before claiming it guards.
...
- A workflow run that fails in ~0 seconds — or a path-filtered workflow whose check never appears on the PR at all — almost always means an *invalid workflow file* (a YAML parse error), not a passing or correctly-skipped check; `gh run view <id>` reports "This run likely failed because of a workflow file issue." Treat it as a hard failure and fix the YAML — never read a missing or 0-second check as green. Parse workflow YAML locally before pushing CI edits (e.g. `python3 -c 'import yaml,sys; yaml.safe_load(open(sys.argv[1]))' <file>`). One easy way to corrupt it: deleting a single `paths:`/sequence entry by matching a *leading* newline (`old="\n      - 'x'"`, `new=""`) consumes the *previous* line's boundary and concatenates two entries onto one line (`- 'a'      - 'b'`), which is invalid YAML. Match the entry *with its own trailing* newline instead, and re-read the region after. Shipped as broken `scripts.yml`/`markdownlint.yml` on PR #140 and cost a force-push.
- Removing the last in-tree file of a language can break language-specific CI that auto-detects languages. CatVox code scanning uses CodeQL **default setup**, which keeps each previously-detected language in its configured list: when a change moves or deletes *all* of a language's files (e.g. the only Python, `tools/ai-loop/*.py`, moving into a submodule that CI checkouts do not initialize), `Analyze (<lang>)` fails with "no source code seen during build." Flag that the CodeQL language list needs updating — repo *Settings → Code security → Code scanning → CodeQL default setup*, which is Ivan's to change — and consider whether scanning for that language should follow the code to where it now lives (the extracted `kathelix/ai-loop` repo took on its own Python CodeQL). PR #140.
...
**Inherited "blocked on upstream" premises need re-verification, not just a version re-check.** When an issue is parked waiting on upstream, its recorded blocker is a claim like any other — and an inherited one that has aged is exactly the kind that goes stale or was never tested. Re-verify the *premise* of the block empirically, not only whether the dependency versions moved. Worked example: issue #72 deferred closing a `uuid` CVE on the stated premise that `uuid@11.1.1` is "ESM-only" and a global `overrides` would break the CJS callers under gaxios v6 / google-gax v4. But `npm view uuid@11.1.1 exports` showed a dual build (`node.require → ./dist/cjs/index.js`, `main` is CJS) and a scratch `require('uuid').v5(...)` ran — the premise was false, and the one-line override that had been deferred for a week closed the alert immediately (PR #132). Corollary: when you *do* legitimately park an issue, record a **falsifiable per-condition check** for each blocker ("overridable once `uuid`'s `exports` exposes a `node.require`"), not a prose verdict ("uuid is ESM-only") — so the next re-check is mechanical and lifts the block the moment it becomes true, instead of inheriting a conclusion no one re-tests.
...
When a guard validates that a built artifact is *safe to ship* — no wrong value leaks in and every required value is present — enumerate the whole problem before coding rather than letting review drive completeness: the *surfaces* the artifact actually embeds (for a Release build: the environment xcconfig, the committed Firebase plist the app loads, the *generated* `Info.plist` composed from `project.yml` `info.properties`, the entitlements file, and the build-configuration bindings) crossed with the *value types* that can be wrong (another environment's identifiers and secrets, debug switches and tokens, and — the dual that is easy to forget — *required* values that are absent or left as unresolved `$(…)` substitutions). A guard has two directions and needs both: it must *reject bad values* AND *require good ones*; a scan that only rejects leaks still passes a build whose required `Info.plist` key was dropped or never resolved, which then fatals at launch or ships a broken endpoint. Verify the surface the artifact *actually embeds*, not a value in isolation: resolve entitlements through the target's `CODE_SIGN_ENTITLEMENTS` rather than a hard-coded path, scan the generated `Info.plist` rather than URLs reconstructed from the xcconfig, and validate the committed plist the Release build actually loads. Worked example: `scripts/validate-release-safety.mjs` (PR #135) only reached this shape after eight automated-review rounds / eleven findings — and the two most severe (an unvalidated protected Firebase plist, and absent or unresolved required `Info.plist` keys) were the last two caught — because it was built surface-by-surface instead of enumerating the grid and both duals up front.

Parse structured configuration (YAML, plist, xcconfig) *structurally*, never by first-match or single-token regex: a regex silently misses a configuration-specific override (`settings.base` vs `settings.configs.Release`) and accepts a prefix collision (capturing `Release` from a `Release-dev` archive config). When adding a parser dependency is not acceptable — the repo's scripts run on pure Node built-ins with no root `package.json` by design — a small scoped block reader beats regex (see `parseProjectYaml` in `scripts/validate-release-safety.mjs`). This is the structured-input counterpart to the identifier-comparison-semantics rule above.
