...
* **Environment Configuration:** CatVox treats the environment name as configuration data, not code, so environments can be added or removed without source-level branching. Each environment owns its own Firebase/GCP, App Check, backend, analytics, CI-auth identity, and Terraform-state configuration, in one of two security tiers (mutable or protected). The canonical rules — the security-tier flag and its invariants, the xcconfig-driven parameterization, and the mutable-vs-protected differences — live in the Environment Model section of `docs/TRD.md`. See ADR-0017, ADR-0018, ADR-0019, ADR-0020, ADR-0021, ADR-0024, ADR-0025, and ADR-0026.
