...
Each environment owns its own GCP/Firebase project, Firebase iOS app, App Check configuration, backend endpoints, PostHog project/token configuration, GitHub Environment secret/variable set, and Terraform state. PostHog API credentials such as `POSTHOG_API_KEY`, `POSTHOG_ORGANIZATION_ID`, `POSTHOG_HOST`, and `POSTHOG_PROJECT_ID` are operational automation inputs only and must not be committed to app-facing configuration.
