...
Each environment owns its own GCP/Firebase project, Firebase iOS app, App Check configuration, backend endpoints, PostHog project/token configuration, GitHub Environment secret/variable set, and Terraform state. Future PostHog Terraform automation should keep operational API inputs out of app-facing configuration and document exact variable names in that Terraform slice.
