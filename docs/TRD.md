...
* [ ] **App Check Console & Live Gate:** Before merging the App Check PR, confirm App Attest is enabled for `com.kathelix.catvox` in Apple Developer, register the Firebase iOS app with App Attest, create/register the Debug Provider token, run a Debug scan with that token, run `make functions-integration` with `CATVOX_APP_CHECK_DEBUG_TOKEN`, and confirm unauthenticated curl calls to both HTTP Functions return `401 app_check_unauthorized`.
